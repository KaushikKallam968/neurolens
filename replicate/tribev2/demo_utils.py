...
class TribeModel(TribeExperiment):
    ...
    def from_pretrained(
        ...
        if checkpoint_dir_path.exists():
            config_path = checkpoint_dir_path / "config.yaml"
            ckpt_path = checkpoint_dir_path / checkpoint_name
        else:
            from huggingface_hub import hf_hub_download

            # Use the original string to preserve forward slashes for HF repo IDs
