...
class ExtractWordsFromAudio(EventsTransform):
    ...
    def _get_transcript_from_audio(wav_filename: Path, language: str) -> pd.DataFrame:
        ...
        with tempfile.TemporaryDirectory() as output_dir:
            logger.info("Running whisperx via system Python (CUDA torch)...")
            # Use system Python directly (has CUDA torch + whisperx installed)
            # with a wrapper that patches torch.load for PyTorch 2.6+ compat
            wrapper_path = str(Path(__file__).parent.parent.parent.parent / "backend" / "whisperx_wrapper.py")
            cmd = [
            ...
            if result.returncode != 0:
                # If whisperx fails (e.g. no speech detected), return empty transcript
                logger.warning(f"whisperx failed (may be no speech): {result.stderr[-200:]}")
